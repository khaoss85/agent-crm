...
| Awesome lists | `docs/marketing/AWESOME_LIST_SUBMISSIONS.md` | `awesome-mcp-servers`: fork and open the PR, the entry line and body are written. `awesome-claude-code`: an **issue form, not a PR** — and its 14-day rule makes it eligible on **2026-08-21**, so sending it earlier spends the one allowed recommendation on a bot rejection | — |
