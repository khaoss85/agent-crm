...
| npm packages | root `package.json` is `private: true`; `packages/create-accordo/package.json` exists and is `private: true` | **Names reserved 2026-08-09**: `accordo@0.0.1` and `create-accordo@0.0.1` are published placeholders that install nothing — verified against the registry, and unchanged since. **The source precondition is met**: `create-accordo` scaffolds a real project from a checkout (`tests/project-bootstrap.test.js`). The release artifact is not built yet: npm cannot include sibling directories through a `files` array, and `npm pack --dry-run ./packages/create-accordo` currently contains only five package-local files, so it would report `FRAMEWORK_SOURCE_UNAVAILABLE`. Still to do: build a staged publish root carrying the framework, install its tarball in isolation and rerun bootstrap → inspect → doctor → tests → smoke; create the `@accordo` **organization** (web-only, no CLI); remove `private` only from the verified artifact; flip `site/brand.json` → `npm.status` in the same commit (`scripts/distribution-check.js` fails while the two disagree); publish with `--provenance`. Until then, no document may say `npm create accordo` creates a project | release packaging + human publication |
