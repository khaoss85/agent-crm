...
| Awesome lists — **four prepared, all eligible now** | `docs/marketing/AWESOME_LIST_SUBMISSIONS.md` | In send order: (1) `punkpeye/awesome-mcp-servers` — fork and open the PR, entry line and body written. (2) `hesreallyhim/awesome-claude-code` — an **issue form, not a PR**; its 14-day rule cleared on **2026-08-18**, corrected on 2026-08-25 from a wrong first-commit date read off a shallow clone. (3) `travisvn/awesome-claude-skills` (14.8k stars) — PR, entry and body written. (4) `sneg55/awesome-open-source-crm` (13 stars) — PR; small, but the only list with a *CRM Frameworks* section, which is the category that matters. Four more lists were assessed and rejected with reasons in the same file. Note that list 2's own CONTRIBUTING requires the recommendation to come from a human — *"resource recommendations must be created by human beings"* — so §10.4 and the destination agree here | — |
