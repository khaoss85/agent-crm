...
31. Under an order, a **Contract activation** section offers a policy selector and **Plan activation**, stating that planning is read-only. Planning lists every order component with its classification *and the reason for it*, and creates no record.
32. A component the policy could not classify is highlighted as blocking, with a type selector and a required reason; no term form and no activation control appear while anything is ambiguous. Applying the decision without a reason is refused before any request leaves the browser.
33. Once nothing is ambiguous, the term form asks for effective date, term start and term end as calendar dates, states that the end date is inclusive and that auto-renew and notice days are recorded only, and states that activating requires a signed-in user actor and that this is **not** Finance or Legal role enforcement.
34. Activating once re-renders to evidence: contract facts (term, day count, deciding policy version and fingerprint, signed document hash), contract lines with each classification's reason and any human override next to what the policy said, the subscription with its per-period line amounts, and pending delivery/service obligations with the statement that nothing executes them. The activation control does not accept a second click.
