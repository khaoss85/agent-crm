...
- **Extension** (b/c): `gemini-extension.json` at the repository root, with `contextFileName` pointing at `GEMINI.md`; an extension may also ship `commands/`, `skills/` and MCP servers ([writing extensions](https://github.com/google-gemini/gemini-cli/blob/main/docs/extensions/writing-extensions.md)). Install current repository source with `gemini extensions install https://github.com/khaoss85/agent-crm --ref main`. Select the ref explicitly: the default installer can prefer the latest GitHub release, which is a separate artifact from current main. No new gallery submission is required.
- **Fresh host receipt (2026-09-07):** Gemini CLI 0.58.0 installed the default URL from GitHub release `v0.1.0`; a separate empty host home with `--ref main` installed git source at `3b51b97fa7ae4ae5cfad3edd97d0ceec8e75a01e` and detected the MCP server and skills. These are maintainer installation checks, not adoption or an agent-build benchmark. The public gallery feed still carried its September 2 metadata at readback; the current source description is corrected, but gallery refresh remains controlled by the platform.
