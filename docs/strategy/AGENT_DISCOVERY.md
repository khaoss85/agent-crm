...
- **Skills** (c): Gemini extensions carry a `skills/` directory of their own. We deliberately do **not** ship a third mirror of the 12 skills — the two existing mirrors (`.claude/skills/`, `.agents/skills/`) already have a divergence-detection check, and a third copy multiplies the sync surface. Wiring skills into the Gemini extension waits for DX2 (skill mirror sync).
