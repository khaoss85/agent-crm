...
| Social preview uploaded and live, but refresh required | the repo page's `og:image` resolves to `repository-images.githubusercontent.com`; visual inspection on 2026-08-10 found the older 421-test artwork |
...
### 2.4 GitHub social preview — **refresh required**

Uploaded by hand through Settings → General → Social preview; GitHub exposes no
API for it. The live 2560×1280 artwork still says 421 tests and names Claude Code
and Codex but not Gemini CLI. Run `npm run site:shots` after the claims receipt is
remeasured, then upload `site/dist/shots/social-preview.png`; the generated frame
uses the current test count and the checked site design.
