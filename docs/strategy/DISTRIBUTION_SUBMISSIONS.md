...
| **Anthropic community marketplace** ([submit](https://platform.claude.com/plugins/submit)) | Form submission; the plugin should point at a live install command before review | `create-accordo@0.1.0` is live and verified |
| **Anthropic Connectors Directory** | Needs a hosted Docs MCP endpoint and a privacy policy | PR #52 merges, the read-only Docs MCP is hosted, and its public contract is re-verified at submission time |
