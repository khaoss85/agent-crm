...
| **Evidence** | `benchmarks/tool-selection/panel-2026-08-13/PANEL.md`, `frozen-protocol.json`, `preregistration.json`, `aggregate.json` — the panel record names the frozen protocol document under `docs/benchmarks/`, which is deliberately not spelled here: its filename is a rubric marker the benchmark's own isolation scan refuses inside a fixture, and this handoff is fixture input. `tests/agent-tool-selection-scoring.test.js`, `tests/agent-tool-selection-prompts.test.js` |
