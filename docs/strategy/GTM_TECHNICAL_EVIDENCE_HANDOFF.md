...
| **PROVE** | Quality Gates, Project Verify (DX5) and Scenario Evidence (DX6, now with two consumers); Implementation Evidence (DX10) still to come | **partial** | `docs/QUALITY_GATES.md`; `crm project verify --json`; `crm scenario run lead-to-won --json` and `crm scenario run service-sla-escalation --json`; DX10 **not built**, so nothing maps a plan's requirements to the code that implements them. A second scenario makes the evidence broader, not complete |
