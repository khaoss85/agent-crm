...
- Once Accordo Cloud exists (`AGENT_CRM_CLOUD.md`; public product target; private pilot evidence is separate), each starter additionally gets a one-approval managed deploy path — a real distribution asset only when the deployed demo is genuinely reproducible by a reader; no Cloud-dependent content ships before the capability does.
