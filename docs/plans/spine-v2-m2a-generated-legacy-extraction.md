...
| `LATER_M2_CORE` | `packages/core/src/action-runtime.js` | Core runtime persistence outside this bounded compatibility family. `packages/core/src/package-registry.js` was listed here at M2A and has since moved behind the definition-version store in M2B (`docs/plans/spine-v2-m2b-definition-version-store.md`). | action suites |
| `EXTRACTED_IN_M2B` | `packages/commercial/src/registry.js`, `packages/signature/src/registry.js`, `packages/intelligence/src/registry.js` | Package definition-version registries were separate package extraction work at M2A. M2B moved all three, with `packages/core/src/package-registry.js`, behind one internal definition-version store (`docs/plans/spine-v2-m2b-definition-version-store.md`). | package characterization suites plus `tests/spine-v2-m2b-definition-version-store.test.js` |
...
from this branch. M2B has since landed the definition-version store, which is
why two rows above are annotated rather than left describing a checkout that has
moved.
