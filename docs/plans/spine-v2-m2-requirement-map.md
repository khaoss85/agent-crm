...
| `M2-04` | The four contract-2 versions — capabilityContract collision resolved | M2E-1 | M2E-1 resolved the collision atomically: capabilityContract now lives on the declaration composition can read, absence normalizes to synchronous v1, and the historical synchronous contract-lifecycle-source@2 interface was corrected from capabilityContract 2 to 1 while its domain capability version remains 2. The c363be2 archaeology and the ratified-vocabulary correction are recorded in the plan decision log and ADR-018 addendum 9; mixed graphs refuse with PACKAGE_ASYNC_CONTRACT_REQUIRED. |
...
| `M2-33` | CLI matrix executed against real PostgreSQL | M3 | Running every supported command against a live PostgreSQL service needs the M3 adapter and a real service container, so this half cannot be executed at M2-complete. The SQLite run, the declared refusals and the classification contract are M2F and are carried by M2-32; splitting them is what keeps the owner column answerable by a reader who does not open the reason. |
