...
| Open PRs | **This one (PR #79, the Commercial extraction) at its merge.** `claude/signature-characterization` holds the Signature & Order LA0 harness and frozen baseline, pushed with no PR: it lands together with the signer input-integrity fix, which must precede the Signature extraction. Earlier this wave: PR #73 closed unmerged (guidance extracted by PR #74), the second frozen panel landed through PR #75 from a rewritten history. |
