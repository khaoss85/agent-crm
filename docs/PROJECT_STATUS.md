...
| Open PRs | GitHub's live PR list is authoritative. M2E and the remaining M2F deployment/configuration slices are active infrastructure work; this status does not infer their current PR numbers. PR #134 is unrelated strategic-roadmap work and is not part of the infrastructure campaign. No measurement runs until M2 is complete; `Measured at` and `site/claims.json` therefore still name `27cc663`, and `measurement.test_tree_current` correctly reads `false`. |
