...
| Open PRs | **PR #95 (Customer Data Foundation v1) at its merge.** Earlier this wave: PR #92 (signed-term integrity verifier) and PR #93 (ledger re-measure) merged. **Production Spine remains unimplemented** — there is still no authentication, tenancy or role enforcement, and every "a human decided" in this repository means an actor object said so inside an audit boundary. |
