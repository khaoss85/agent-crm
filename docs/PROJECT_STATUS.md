...
| Open PRs | **None.** PR #73 (rail-selection guidance) was closed unmerged after its two-file guidance section was extracted and merged by PR #74; the second tool-selection panel it also carried — the guidance re-run, receipts and record — stays on `claude/rail-selection-guidance-v2` awaiting a PR of its own, with the explicit receipt-shape decision that a receipt is six evidence files and never the apparatus config tree. |
...
the Agent Experience track beyond AX2 — except the AX3 tool-selection
instrument, which exists and has run under `benchmarks/tool-selection/`
(observation only: no product surface, no published number, `comparative: false`);
DX2 and DX9.
