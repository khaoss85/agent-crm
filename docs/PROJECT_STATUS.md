...
| Open PRs | GitHub's live PR list is authoritative. At this snapshot PR #139 is a bounded M2C evidence follow-up; M2E-2/M2E-3 and M2F remain unmerged. PR #134 is unrelated strategic-roadmap work and is not part of the infrastructure campaign. No measurement runs until M2 is complete; `Measured at` and `site/claims.json` therefore still name `27cc663`, and `measurement.test_tree_current` correctly reads `false`. |
