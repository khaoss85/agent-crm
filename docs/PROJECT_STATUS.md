...
| Open PRs | The post-M2C measurement PR, which will record the merge commit and re-measure the suite. Until it does, `Measured at` and `site/claims.json` `measuredAgainst` both name `27cc663`, and `measurement.test_tree_current` correctly reads `false`. |
