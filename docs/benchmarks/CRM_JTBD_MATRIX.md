...
- **Status**: **partially supported** — the *tracking* half is now reusable and the *scheduling* half still does not exist. Work v1 (ADR-030) replaced the bespoke Lead slice with one package-native Task and Activity model that three business events consume, over an explicit transition table with a human-actor boundary. Nothing is **scheduled**: there is no scheduler, no delayed-workflow primitive, no reminder, no queue, no recurring work, and `dueAt` is evidence that changes no state — the word *schedule* in this row's title is still unsupported.
- **Evidence**: `tests/work-operations-e2e.test.js` (both consumers, the transition table, both terminal states, idempotent replay and a named 409 on a divergent payload, and an injected clock stepped a year past `dueAt` leaving the row byte-identical), `tests/work-operations-evidence.test.js` (fault injection after every write, two-connection race, exact reads past 500 rows, hostile input), `tests/lead-qualification-e2e.test.js`, `tests/work-package-absence.test.js`, `tests/work-provenance-and-actor.test.js` (provenance is bound to the consumer the registry resolved; an actor identity is never truncated into another person's), `tests/work-source-key-stability.test.js` (each consumer's key is stable across two clock instants), `docs/WORK_TASKS.md`.
- **Manual interventions**: everything time-based. Nothing fires, nothing reminds, and a person opens, completes or cancels every task.
...
| JTBD-LI-06 | Manually reassign with permission and reason | **not supported** | the assignment record carries `source`/`previousAssignmentId`/`reason` as future data, but no override action exists, and none may claim a manager made the call until a verifier makes that manager a verified person rather than an asserted one (JTBD-15) |
...
| JTBD-CO-04 | Obtain required commercial approval on a discount | **partially supported** | the **human-actor boundary is validated** (agent actors refused 403; one atomic decision per version; concurrent decisions resolve to one winner) — but `requiredApprovalKey` is still a label: **secure Sales-Manager/Finance role enforcement is not validated**, because ADR-038 deliberately left approval keys descriptive rather than promote them into enforced permissions, which would change the meaning of records already written (JTBD-15) |
...
### Delivery & Service (handover at M13, execution at M14a, economics at M14b1, and change, deliverables and acceptance at M14b2 — all merged; `docs/DELIVERY_HANDOVER.md`, `docs/DELIVERY_ECONOMICS.md`, `docs/DELIVERY_CHANGE_ACCEPTANCE.md`)
...
| JTBD-DS-04 | Restrict partner access to assigned work only | **not supported** | authorization and tenancy now exist, but no partner access surface does: M13 records a partner engagement and grants no access to restrict, and no verifier ships to authenticate a partner (JTBD-15) |
...
| JTBD-CS-03 | Amend seats or quantity on a live subscription | **not supported** | still no in-place amendment primitive, and M16b deliberately does not add one: a quantity change produces a **successor agreement** from its own signed Order, and the live subscription it succeeds is never modified. Reading this row as satisfied would mean claiming that seats moved on the subscription somebody is already on, which is exactly what M16b refuses to do |
| JTBD-CS-04 | Record an expansion or contraction, classified at the time of change | **partially supported** | M16b executes a governed successor agreement from a signed Order and records the change with a classification **derived from the immutable line delta** — `renewal`, `expansion`, `contraction`, `mixed`, or `commercial_change` when a narrower label cannot be derived — never a client-supplied label. The exact per-line before/after travels with it, grouped by currency and full recurrence. **Partial, and narrowly so**: quantity direction is derivable only for components that carry a quantity, so a flat-fee line whose amount moved is honestly `commercial_change` rather than an expansion; a price uplift with no quantity movement is deliberately `commercial_change` too; nothing is billed, scheduled or notified, and no MRR/ARR/TCV is computed. Evidence: `tests/lifecycle-amendment-execution-e2e.test.js`, `tests/lifecycle-amendment-execution.test.js`, `tests/admin-lifecycle.test.js`, `docs/RENEWAL_AMENDMENT.md` |
...
| JTBD-CS-08 | Apply a versioned price-uplift policy at renewal | **not supported** | unchanged by M16b. The versioned-policy mechanism exists (ADR-015/016/018) and M16b runs the *activation* policy over the successor's components, but it computes no price and applies no uplift: every amount is copied from the signed Order, the live catalog is never read, and a renewal whose amounts moved without a quantity moving is classified `commercial_change` rather than priced. Uplift needs pricing, which is Commercial's job on the quote that precedes the signature |
| JTBD-CS-09 | Cancel or non-renew with an audited reason | **partially supported** | the **non-renew half only, as recorded intent**. M16a's `record-renewal-decision` stores an immutable `not_renewing` decision with a required human reason, its actor, the term boundary it was taken against and that term's declared provenance. It ends no contract, cancels no subscription, stops no service and recognizes no churn — there are still no cancellation primitives, and the cancel half of this row is **not supported**. Evidence: `tests/lifecycle-renewal-operations-e2e.test.js`, `tests/lifecycle-renewal-operations.test.js` |
| JTBD-CS-10 | Read a complete amendment history | **partially supported** | M16b records an immutable `contract-succession` row per amendment: source and successor contract, version, subscription and Order, the derived classification with its basis, the exact line delta, both terms with their provenance, the term continuity, the signed document hash, the deciding policy version and fingerprint, and who executed it when. Lineage is 1:1:1 and DB-enforced, so the chain is unambiguous. **Partial**: it is a *successor chain*, not a field-level amendment log — there is no in-place amendment to log — the chain is linear (one successor per agreement), and it starts at M16b: agreements succeeded before it exist have no lineage row and none is backfilled. Evidence: `tests/lifecycle-amendment-execution-e2e.test.js`, `tests/lifecycle-amendment-execution.test.js`, `tests/admin-lifecycle.test.js`, `docs/RENEWAL_AMENDMENT.md` |

### Data operations (Customer Data Foundation v1 — ADR-037, `packages/customer-data/README.md`)

The everyday jobs that decide whether a CRM is usable at all. **Three of the nine now have a foundation**; the other six remain unimplemented and unassigned, which is still a finding. Every promotion below is backed by the `customer-identity-governance` scenario run and its journey, and each is *partial* on a stated reading — a promotion whose note does not say what it still cannot do is a promotion nobody should trust.

| ID | Job | Status | Notes |
|---|---|---|---|
| JTBD-DO-01 | Import records from CSV | **partially supported** | Customer Data Foundation v1 adds a bounded import under a fingerprinted mapping (`customer-rows@1`): a preview that provably writes nothing, a per-row receipt carrying an outcome and a reason code, counts that always reconcile (`accepted + rejected + skipped == rows`), an all-or-nothing or partial acceptance mode, and an idempotency key derived from the source system, the mapping fingerprint and the sorted row digests — never a clock or a random value, so the same import retried is the same run. `examples/scenarios/customer-identity-governance.scenario.json`, `tests/customer-data-foundation.test.js`. **Still partial, and the row's own word is the gap**: there is no CSV. The rows are a bounded JSON array a caller hands in, capped at 500, with no file upload, no delimiter/encoding/header inference, no column-mapping UI, no streaming and no resumable or scheduled import |
| JTBD-DO-02 | Detect duplicates on import or entry | **partially supported** | deterministic matching under a versioned, fingerprinted policy (`deterministic-customer-match@1`) with three exact rules in a fixed precedence — external identity, contact email, company name + domain. An exact match reuses the existing record across source systems; ambiguity produces a `duplicate-candidate` carrying its rule, its evidence and the policy fingerprint, and the row is left unresolved for a person. The scenario earns this by *attempting* an ambiguous row and publishing that no tie was broken. **Still partial**: nothing fuzzy, phonetic, probabilistic or machine-learned exists, there is no score or threshold, no rule breaks a tie, and detection runs **on import only** — entering a record by hand through a service or action detects nothing |
| JTBD-DO-03 | Merge two Companies or Contacts | **partially supported** | on a deliberately narrow reading: a **logical** canonical merge. A human — never an agent, never without a reason — records that two records are one customer, producing a canonical/alias cluster with the deciding identity and reason on every link. After it, both records still exist **byte-identical**, both still resolve, both read one consolidated profile, and neither a decided candidate nor an already-clustered record can be silently re-decided or re-parented. **Still partial, and the row's own word is again the gap**: there is no **physical** merge. Nothing is deleted, no reference is re-pointed, no field is survivorship-resolved and nothing is un-mergeable by audit — an operator who wants two rows to become one row does not get that here, and it is deliberately deferred to Customer Data Operations v2 |
...
| JTBD-CM-03 | Log meetings and calls | **not supported** | Work v1 adds a free-text **note** on a subject timeline, and that is all: there is no meeting, call, participant, duration, direction or outcome model, no calendar and no telephony. A note is not a logged call |
| JTBD-CM-04 | Schedule and track a follow-up | **partially supported** | **tracked, never scheduled.** Work v1 (ADR-030): one reusable Task and Activity model with an explicit transition table, consumed by Lead qualification, Lifecycle's commercial follow-up and Service escalation, each opening exactly one task under a deterministic business key inside its own transaction, and refusing to write at all when that transaction is not there — `tests/work-operations-e2e.test.js`, `tests/work-operations-evidence.test.js`, `tests/work-provenance-and-actor.test.js`, `tests/work-source-key-stability.test.js`. **Scheduling is still absent**: no scheduler, no reminder, no notification and no assignment, and `dueAt` changes no state — the same limit JTBD-07 states |
...
| JTBD-DG-04 | Anonymize or delete a subject while preserving immutable evidence | **not supported** | the hardest design in the track: commercial evidence is deliberately immutable. Customer Data Foundation v1 does **not** move this row and does not claim to: it anonymizes and deletes nothing, its canonical link deletes nothing, and governing a data-quality finding records a decision while keeping the finding and its evidence. The only related fact it contributes is a narrow negative one — no raw provider payload is retained from an import — which is a smaller claim than this row asks for and is not a step toward it |
...
- Deliberately **not** marked validated: pipeline for custom objects, **scheduling** (Work v1 makes the *task* reusable across three business events and schedules nothing at all — no scheduler, no reminder, no recurring work, and `dueAt` moves no state, so JTBD-07 stays partial), onboarding, churn/upsell, reporting, integrations, permissions. Primitives for some exist, but no end-to-end proof does.

This matrix guides roadmap prioritization: the largest gaps blocking common CRM adoption are **scheduling** (Work v1 shipped the reusable Activity/Task half; the timer, reminder and queue half needs `JOBS_AND_OUTBOX.md` — JTBD-07), generated workflows/approvals for custom objects (JTBD-06), and the authentication prerequisite that tenancy and authorization now sit on top of (JTBD-15).

The workstream sections chart the M9–M16 roadmap (corrected at the Platform Alignment Gate: M12 is Order Activation & Subscription, and Delivery/Service/Analytics shift by one — `EXECUTION_ROADMAP.md`). Commercial Operations (M10) is now implemented for the local slice: JTBD-CO-01/03 **validated end to end**, CO-02 partial (fixture provider only — no real external catalog), CO-04 partial (human boundary validated, secure roles not), CO-05/06/07 still **not supported** (Milestone 11). Lead Intelligence (M9) is implemented for the local slice: JTBD-LI-01/02/04/07 **validated end to end** (fixture provider, explainable versioned scoring, deterministic routing, persisted version fingerprints — ADR-015), LI-03/05/08/09 partial, LI-06 still not supported. The CO/DS/AN sections remain all **not supported** except JTBD-CO-04 and JTBD-DS-01, which inherit *partial* status from the validated approval and workflow primitives. Signature and Order (M11) is implemented for the local slice: JTBD-CO-07 **validated end to end**, CO-05/CO-06 partial (fixture provider, test-only webhook key, provider-reported artifact hash — ADR-017). Contract activation (M12) is implemented for the local slice as the first optional domain package, and its adversarial review narrowed what it may claim: JTBD-CS-01 is **partially supported** (the activation is real and atomic, but the term is post-signature operational metadata, not a signed term), CS-02 partial (an activation record that now keeps every recurring commitment, with nothing billing, renewing or cancelling it), CS-05 deliberately still **not supported**, and every remaining CS row unchanged. Renewal & expansion operations (M16a) is implemented for the local slice as a package that consumes another through a declared capability and offers none of its own: it moves **only** JTBD-CS-09 to *partially supported*, and only its non-renew half, as recorded intent with a reason. CS-03, CS-04, CS-06, CS-07, CS-08 and CS-10 were unchanged by M16a — it renews nothing, cancels nothing, signs nothing, schedules nothing, prices nothing and records no successor, so no other row inherited status from it. **Renewal & amendment execution (M16b)** is its execution counterpart and moves exactly two rows: JTBD-CS-04 and JTBD-CS-10 to *partially supported*, on a governed successor agreement built only from signed evidence, with a classification derived from the immutable line delta and immutable 1:1:1 lineage. **No neighbouring row moves with them.** CS-03 stays *not supported* because M16b deliberately does not amend a live subscription in place; CS-06 and CS-07 stay *not supported* because there is still no scheduler and nothing renews on a date; CS-08 stays *not supported* because no price is computed anywhere; CS-05 stays *not supported* because no MRR/ARR/TCV is derived; and CS-09's cancel half stays *not supported* because succeeding an agreement is not cancelling it. Delivery handover (M13) is implemented for the local slice as the second package — and the first that depends on another through a declared capability: JTBD-DS-01 **validated end to end**, DS-02/DS-03/DS-05 partial (a planned project, a planned partner reference that grants nothing, and a milestone plan), DS-04 and DS-06 through DS-11 still **not supported**. Custom Package Authoring v1 adds a **Package developer** section: JTBD-PK-01/PK-02 **validated end to end** (a customer-authored package attaches and detaches with the kernel fingerprinted unchanged; cross-package reach only through a declared capability), PK-03/PK-04 partial (the CLI executes what it validates; removal leaves data behind with no uninstall), and PK-06/PK-07 **not supported** — no registry or marketplace, and no sandboxing of package code. **PK-05 moved to validated end to end** when DX3 shipped `accordo package scaffold`, on the merged flow `scaffold --apply` → `validate` → `inspect` → `package test`; it is a minimal empty local package, dry-run by default, that invents no business logic, composes nothing, verifies no global identity uniqueness and installs or publishes nothing. **No neighbouring row moved with it**: PK-06 and PK-07 are unchanged, because a scaffold is not a registry and an empty package is not a sandbox. The adversarial review of PR #17 corrected the contract before these rows were written: ADR-018 addendum 4. Marketing & Growth Operations (MK0–MK7) is added as a **design-only parallel workstream**: all 43 JTBD-MK rows are **not supported**, because no campaign, audience, consent-check, journey, experiment, content, provider, media-plan, funnel or attribution primitive exists. Lead Intelligence's scoring and routing is not a marketing campaign system and no MK row inherits status from it. A cross-cutting **Agent experience** section (AX0–AX5) records how a user reaches any of this from a business objective: JTBD-AX-01 and AX-02 **validated end to end** (deterministic application inspection through `accordo app inspect`, and extending without patching the kernel), AX-03 moves to **partially supported** with AX2's machine-readable Solution Plan contract (a document that can be checked and bound to a real inspection — not a planner and not a runtime), AX-04/AX-05/AX-06 partial, and AX-07 through AX-10 **not supported** — there is still no Solution Plan *runtime*, no funnel or campaign primitive, and no production deployment path. DX10 (`accordo solution verify`) strengthens the *evidence* behind AX-05 and AX-06 and moves **neither row**: it maps a plan's requirements to authorities that already decide, and the first honest answer it produced is that the one plan this repository declares current is not implemented. A rung that reports a gap does not close it, and no neighbouring row inherits status from it.
...
**Six sections added at the Platform Alignment Gate** — Contract & Subscription, Data operations, Communications, Data governance, Cloud operations and Design-to-CRM — are **entirely `not supported`** except three rows that inherit *partial* status from proven primitives (JTBD-CM-04, the reusable deterministic follow-up Task of Work v1; JTBD-DO-08, its per-subject Activity timeline, which is deliberately **not** unified across domains; JTBD-DC-04, the Admin override seam). Data operations is the most uncomfortable of them: CSV import, dedupe, merge, export, bulk edit, saved views, global search and an activity timeline are table stakes in every commercial CRM and none of them has a milestone. That is recorded here deliberately rather than left implicit.
