...
- **Status**: **partially supported** — Work supplies reusable Tasks and Activity. Explicitly composed self-host timer consumers can open a Work ask through an application-started worker; dueAt alone starts nothing. No recurrence, notification delivery, assignment automation or managed worker service is included.
...
- **Manual interventions**: compose and explicitly start the worker for scheduled asks. A timer opens an ask, never completes or cancels it; notification delivery is not included.
...
- **Status**: **partially supported** — contract activation, human renewal intent and governed successor execution exist, retaining signed-term provenance and immutable lineage. Timer consumers can open lifecycle-review asks when an application explicitly starts its worker. They do not automatically renew, cancel, bill or notify; historical contracts are not edited.
- **Evidence**: `tests/contracts-activation-e2e.test.js`, `docs/CONTRACT_ACTIVATION.md`, `npm run smoke`, `tests/workflow.test.js`, `tests/lifecycle-amendment-execution-e2e.test.js`, `docs/RENEWAL_AMENDMENT.md`.
...
signed-term provenance, recorded renewal decisions and governed successor
agreements are implemented within the detailed rows below. A successor leaves
the existing subscription unchanged: there is no in-place amendment, automatic
renewal, cancellation, billing or recurring-revenue calculation. Evidence:
`tests/signed-terms-e2e.test.js`, `tests/lifecycle-renewal-operations-e2e.test.js`,
...
| JTBD-CS-02 | Activate a subscription with lines and an initial term | **partially supported** | one subscription per contract, with a line for every component whose commercial axis is `subscription` — including recurring components that also carry a service or delivery obligation, so no recurring commitment is lost. Its term inherits the signed Order snapshot when present, or explicitly reasoned operational metadata for a termless Order; the provenance remains visible. **Still partial**: activation creates commercial records, not billing, proration or cancellation. A future-dated subscription is `scheduled`; self-host timers can request lifecycle review but do not automatically activate or renew it. Governed amendment execution creates a separate successor agreement without modifying the existing subscription. Evidence: `tests/signed-terms-e2e.test.js`, `tests/contracts-activation-e2e.test.js`, `tests/lifecycle-amendment-execution-e2e.test.js` |
...
| JTBD-CS-06 | Schedule a renewal ahead of term end | **not supported** | self-host timer consumers can open a lifecycle review ask when an application explicitly starts its worker, but full renewal scheduling and execution are not established by that primitive; automatic renewal and delivered notices remain absent (`JOBS_AND_OUTBOX.md`) |
...
The everyday jobs that decide whether a CRM is usable at all. **Five of the nine now have a foundation**; the other four remain unimplemented and unassigned, which is still a finding. Every promotion below is backed by the `customer-identity-governance` scenario run and its journey — and DO-04 and DO-05 additionally by `tests/customer-data-operations-v2.test.js` — and each is *partial* on a stated reading — a promotion whose note does not say what it still cannot do is a promotion nobody should trust.
...
| JTBD-CM-04 | Schedule and track a follow-up | **partially supported** | Work provides one reusable Task and Activity model. Self-host timer consumers can open scheduled Work asks through an explicitly started worker; dueAt alone starts nothing. Still partial: no recurrence, notification delivery, automatic assignment or managed worker service. See `tests/work-operations-e2e.test.js`, `tests/work-operations-evidence.test.js`, `tests/work-provenance-and-actor.test.js`, `tests/work-source-key-stability.test.js` and `tests/spine-v3c-timer-consumers-postgresql.test.js`. |
...
| JTBD-DG-05 | Apply a retention policy and evidence that it ran | **not supported** | retention policy and evidence-producing retention execution are not implemented; generic durable jobs and scheduled asks do not supply them |
...
- Partial or unsupported jobs remain so unless their own end-to-end evidence supports promotion. Work scheduling now has bounded self-host timer primitives, but no recurrence, managed worker or notification service; full renewal scheduling and execution are not inferred from those primitives.
...
The Platform Alignment Gate introduced Contract & Subscription, Data operations, Communications, Data governance, Cloud operations and Design-to-CRM as separate tracks. Their current statuses are the individual rows above. Customer Data Foundation now partially supports bounded JSON import, deterministic duplicate matching and human-governed logical canonical links; CSV parsing, physical merge and complete export/erasure remain gaps. Self-host timers and durable workers are bounded runtime primitives, not blanket validation of communications or lifecycle scheduling jobs.
