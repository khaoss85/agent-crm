...
| JTBD-CS-08 | Apply a versioned price-uplift policy at renewal | **not supported** | the versioned-policy mechanism exists (ADR-015/016/018) and M12 uses it to classify. M16a added the renewal domain, but it declares **no policy at all** and computes no price: uplift needs pricing, which lives in Commercial and is unreachable from a package |
