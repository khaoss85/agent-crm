...
| JTBD-CM-04 | Schedule and track a follow-up | **partially supported** | **tracked, never scheduled.** Work v1 (ADR-030): one reusable Task and Activity model with an explicit transition table, consumed by Lead qualification, Lifecycle's commercial follow-up and Service escalation, each opening exactly one task under a deterministic business key inside its own transaction — `tests/work-operations-e2e.test.js`, `tests/work-operations-evidence.test.js`. **Scheduling is still absent**: no scheduler, no reminder, no notification and no assignment, and `dueAt` changes no state — the same limit JTBD-07 states |
