...
### Delivery & Service (Delivery handover implemented for the local slice at M13 — ADR-018 addendum 3, `docs/DELIVERY_HANDOVER.md`; the rest target M14–M15)
...
| JTBD-DS-02 | Create a Commessa / Delivery Project from an Order | **partially supported** | one Delivery Project per activated contract, with one work package per delivery obligation and a milestone plan, each carrying its own snapshot. It is a **planned** container: status `pending_kickoff`, no execution, progress, scheduling, staffing, cost or billing, and a project is created from the Contract rather than directly from the Order |
...
| JTBD-DS-05 | Manage milestones and deliverables | **partially supported** | a canonical, ordered milestone **plan** is created with the project and linked to its work packages. Nothing schedules, progresses, completes, accepts or bills a milestone, and it is never a contractual milestone |
