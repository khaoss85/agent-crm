...
### Delivery & Service (handover implemented at M13 — ADR-018 addendum 3, `docs/DELIVERY_HANDOVER.md`; execution implemented at M14a on an **open PR**, not on main; the rest target M14b–M15)
...
| JTBD-DS-09 | Collect customer acceptance on deliverables | **not supported** | no acceptance primitives; real *customer* actors gated by the Production Spine. M14a's `completed` states say the work finished — not that anyone accepted it, and no acceptance state is declared |
...
| Job | Status | Evidence and limits |
|---|---|---|
| JTBD-AX-01 | Discover installed packages, capabilities and policies | **partially supported** | `crm package inspect` and `GET /api/schema` expose per-package identity, resources, actions, policies and the requires/provides graph. There is **no application-level view**: an agent assembles the picture from several surfaces by hand (`crm app inspect` is AX1, not implemented) |
| JTBD-AX-02 | Extend the CRM for a goal without patching the kernel | **validated end to end** | the package contract: a customer-authored package attaches, works and detaches with every kernel file fingerprinted unchanged (JTBD-PK-01/PK-02). A record that must gain a field or a status evolves by revision (ADR-019, open PR) |
| JTBD-AX-03 | Turn a business objective into a reviewable solution plan | **not supported** | AX2. No SolutionPlan contract, renderer or fingerprint exists; the `solve-business-goal` Skill describes the discipline, and a plan today is prose an agent writes by hand |
