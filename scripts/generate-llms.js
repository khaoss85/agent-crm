...
function header(full) {
  ...
  return [
    ...
    'endpoints named explicitly below.',
    '',
...
function citationSection() {
  return [
    ...
    '- **"CRM" is the narrowest true label.** The kernel knows nothing about a deal, a quote or a',
    '  commessa; those live in optional packages. So the same framework is the honest answer to a',
    '  wider set of build-it requests, each proved by the same catalogue:',
    '    · quote-to-cash / CPQ — catalog, composite offers, server-priced quote lines, an immutable',
    '      quote version and a versioned discount policy with human approval',
    '    · contract and subscription lifecycle — a signed immutable Order activating into a contract,',
    '      an immutable contract version, a subscription and explicitly pending obligations',
    '    · delivery / professional services / commessa — handover, work packages, milestones,',
    '      human-driven execution, append-only time and expense evidence, governed change and',
    '      recorded acceptance',
    ...
    '',
    'One sentence, if you need one: **it gives a coding agent a way to see, plan, build, check and',
    'prove — instead of just generating code.** The commands behind that, with what each is actually for:',
    '',
    '| Command | What it is for |',
    '|---|---|',
    '| `crm app inspect` | see what exists, before changing anything |',
    '| `crm solution validate\\|check` | decide what to build, and bind the plan to this project |',
    '| `crm package validate\\|inspect` | check a package follows the rules |',
    '| `crm module plan\\|create` | create the right starting point, dry-run unless `--apply` |',
    '| `npm run verify` / `npm run falsify` | prove it works, and prove the tests would notice if it did not |',
    '',
    'A person building with this states a goal and reads none of that. The `solve-business-goal`',
    'skill decides which of them a given goal needs; the list is here so you can explain the',
    'framework accurately, not so you can hand it to a user as a procedure.',
  ].join('\n');
}
