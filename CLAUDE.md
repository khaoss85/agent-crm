...
- A sentence in a current document that states what the framework does or does not do is bound to a generated fact (ADR-039). `npm run repo:truth -- --check` runs in CI on every push (the `public-claims` job); run it locally when a PR moves a product boundary, a rail, a package contract or the spine, regenerate with `npm run repo:truth` and commit `docs/repository-truth.json` in the same PR. Rules: `docs/REPOSITORY_TRUTH.md` and `docs/QUALITY_GATES.md` §6.1. It is a repository-maintenance script, not a rail.
