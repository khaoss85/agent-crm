 ...
 * identifier — a cost category, a partner engagement, a work-package delivery
 * mode — to a rate per hour in a currency.
 *
...
/**
 * A rate per **hour** in minor units. Businesses quote hourly rates, so that is
 * what a policy returns; the cost of a duration divides by 60 under an explicit
 * rounding rule (`economics.js`) rather than hiding a per-minute conversion the
 * operator never wrote down. Ten million minor units an hour is already absurd.
 */
export const MAX_RATE_PER_HOUR = 10_000_000;
...
export function resolveRate({ policy, fingerprint, input, expectedCurrency }) {
  ...
  const { ratePerHourCents, currency, reason } = result;
  if (!Number.isSafeInteger(ratePerHourCents) || ratePerHourCents < 0 || ratePerHourCents > MAX_RATE_PER_HOUR) {
    throw new ValidationError(
      `Delivery cost policy "${policy.name}@${policy.version}": ratePerHourCents must be a safe integer between 0 and ${MAX_RATE_PER_HOUR}`,
    );
  }
