...
const LIMITATIONS = Object.freeze([
  ...
  ['PRODUCTION_SPINE_ABSENT', 'Production Spine v1 (ADR-038) adds verified identity, organizations, memberships and authorization — but no runtime authorization can be reported from SOURCE: which mode a deployment chose, whether a verifier is configured and who holds which membership are all runtime facts. Tenant isolation is DECLARED and NOT enforced: the tenant-storage boundary exists and nothing wires it, so organizations composed into one application share one database and cross-tenant CRM reads and writes are possible — a running application publishes that as TENANT_ISOLATION_NOT_ENFORCED. The rest of the spine is still absent: PostgreSQL and shared-database row-level tenancy, durable jobs, secrets, backups and deployment. This is not a production-readiness statement'],
