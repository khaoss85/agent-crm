...
const LIMITATIONS = Object.freeze([
  ...
  ['PRODUCTION_SPINE_ABSENT', 'Production Spine v1 (ADR-038) adds verified identity, organizations, memberships, server-authoritative authorization and one-tenant-per-instance storage isolation — but none of it can be reported from SOURCE: which mode a deployment chose, which tenant it is bound to, whether a verifier is configured and who holds which membership are all runtime facts, published by a running application at /api/schema. One application instance serves exactly one tenant; two tenants means two instances, and a configuration naming two is refused at startup. Dedicated-database PostgreSQL composition and a bounded self-host secret-provider contract exist; shared-database row-level tenancy, durable jobs, managed secret custody/service, backups and deployment are absent. This is not a production-readiness statement'],
