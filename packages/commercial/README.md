...
- `commercial-quotes@2` — read-only quote/version evidence for the packages
  downstream of a sale (Signature/Order today; amendment execution and
  delivery commercial-followup when they exist), plus the authoritative
  signed-term verifier. The rows are the contract. (`commercial-quotes@1`
  retired: its reads live on byte-identically inside `@2`.)
