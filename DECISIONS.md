...
work. The v1 factory does not import bundled v2 graphs by default
(`createX()` remains contract 1). A uniform contract-2 list can still register
there so `crm package test` can boot a v2 fixture; a mixed v1/v2 list refuses.
The public async factory's default empty contract-2 graph stays empty; callers
pass a uniform bundled v2 selected graph explicitly. Default `accordo serve`,
PostgreSQL, and retiring v1 remain later work.
<!-- truth: spine.postgresql.implemented=absent -->
