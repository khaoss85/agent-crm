...
- [x] **M14b1 — Delivery economics.** A versioned fingerprinted cost policy, append-only time and expense evidence, an immutable versioned economic plan and a reproducible contribution estimate grouped by currency, plus the `delivery-economics@1` capability (`docs/DELIVERY_ECONOMICS.md`). Open PR, awaiting the adversarial review.
- [ ] **M14b2 — Delivery change requests, deliverables and acceptance.** Split out of M14b because four models with four different invariants is not one reviewable PR. Scope, intact: governed non-commercial replans, the `pending_commercial_followup` handoff that never touches a signed Order, deliverable evidence, and recorded customer acceptance — which is evidence a local user actor recorded, never an authenticated customer, a legal signature or a billing trigger. **The next Delivery-domain milestone.**
