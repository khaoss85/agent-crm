...
import {
  DOMAIN_MANIFESTS, DOMAIN_MODULES, POLICY, TERM, boot, project, signedOrder,
} from './helpers/contracts-project.js';

const repoRoot = fileURLToPath(new URL('..', import.meta.url));

/**
 * The standard activation input: the three calendar dates plus the reason the
 * term exists at all. The term is recorded AFTER signature (the signed
 * document carries none), so the package requires a human to say where it came
 * from — see tests/contracts-review.test.js for that rule's own regressions.
 */
const ACTIVATION = { ...TERM, termsReason: 'agreed with the customer after signature' };
...
test('signed order → plan → human activation → contract, subscription and obligations', async (t) => {
  ...
  assert.deepEqual(plan.counts, { subscriptionLines: 3, delivery: 1, service: 1, noObligation: 3, ambiguous: 0 });
  assert.deepEqual(plan.requiredInputs, ['effectiveDate', 'termStartDate', 'termEndDate', 'termsReason']);
  ...
  await assert.rejects(
    () => context.agentClient.module('order').action(order.id, 'activate-contract', { ...POLICY, ...ACTIVATION }),
  ...
  const activated = await client.module('order').action(order.id, 'activate-contract', {
    ...POLICY, ...ACTIVATION, autoRenew: true, renewalNoticeDays: 90,
  });
  assert.deepEqual(activated.result.counts, { subscriptionLines: 3, delivery: 1, service: 1, noObligation: 3, ambiguous: 0 });
  ...
  for (const component of components) {
    ...
    assert.equal(line.commercialOverridden, false);
    assert.equal(line.obligationsOverridden, false);
  ...
  // Platform, seats AND annual support: a recurring service commitment is
  // still recurring money, and it is a subscription line as well as a service
  // obligation (review finding C2).
  assert.equal(subscriptionLines.length, 3);
  ...
  // A recurring per-unit API component owes nothing and is not a recurring
  // right: it is a contract line and NOT a subscription line — recurrence
  // alone decided neither axis.
  const other = lines.find((line) => line.commercialActivation === 'non_subscription' && JSON.parse(line.obligationsJson).length === 0);
  ...
  // The same component carries both consequences, pointing at one contract line.
  const supportLine = lines.find((line) => String(line.componentKey).endsWith('support-fee'));
  assert.equal(service[0].contractLineId, supportLine.id);
  assert.ok(subscriptionLines.some((line) => line.contractLineId === supportLine.id));
  ...
  assert.equal(restarted.modules.get('subscription-line').service.listWhere({ subscriptionId: subscription.id }).length, 3);
});
...
test('ambiguity blocks activation until a human overrides it with a reason', async (t) => {
  ...
  for (const [override, expected] of [
    [{ orderComponentId: ambiguous.orderComponentId, dimension: 'commercial', value: 'subscription' }, /reason is required/],
    [{ orderComponentId: ambiguous.orderComponentId, dimension: 'commercial', value: 'ambiguous', reason: 'because' }, /value must be one of/],
    [{ orderComponentId: 'not-of-this-order', dimension: 'commercial', value: 'subscription', reason: 'because' }, /another order/],
  ]) {
    await assert.rejects(
      () => client.module('order').action(order.id, 'activate-contract', { ...POLICY, ...ACTIVATION, classificationOverrides: [override] }),
      expected,
    );
  }
  ...
  await assert.rejects(
    () => client.module('order').action(order.id, 'activate-contract', {
      ...
      classificationOverrides: [
        { orderComponentId: ambiguous.orderComponentId, dimension: 'commercial', value: 'subscription', reason: 'storage is a subscription' },
        { orderComponentId: setup.orderComponentId, dimension: 'commercial', value: 'subscription', reason: 'wishful thinking' },
      ],
    }),
  ...
  await assert.rejects(
    () => context.agentClient.module('order').action(order.id, 'activate-contract', {
      ...POLICY, ...ACTIVATION,
      classificationOverrides: [{ orderComponentId: ambiguous.orderComponentId, dimension: 'commercial', value: 'subscription', reason: 'bot decided' }],
    }),
  ...
  const activated = await client.module('order').action(order.id, 'activate-contract', {
    ...POLICY, ...ACTIVATION,
    classificationOverrides: [
      { orderComponentId: ambiguous.orderComponentId, dimension: 'commercial', value: 'subscription', reason: 'storage is sold as a recurring right' },
      { orderComponentId: ambiguous.orderComponentId, dimension: 'obligations', value: [], reason: 'storage owes no delivery or service work' },
    ],
  });
  ...
  assert.equal(overridden.commercialActivation, 'subscription');
  assert.equal(overridden.policyCommercialActivation, 'ambiguous', 'the policy decision is preserved next to the override');
  assert.equal(overridden.commercialOverrideReason, 'storage is sold as a recurring right');
  assert.equal(overridden.overriddenBy, 'e2e');
  assert.equal(app.modules.get('contract-activation').service.listWhere({ orderId: order.id })[0].overrideCount, 2, 'both dimension decisions are counted');
  ...
  assert.equal(v2Plan.counts.subscriptionLines, 1);
...
test('activation is atomic, idempotent under concurrency, and source-validated', async (t) => {
  ...
  for (const [patch, code] of [
    ...
    await withOrder(patch, () => assert.rejects(
      () => app.runAction({ module: 'order', action: 'activate-contract', recordId: unsignedOrder.id, input: { ...POLICY, ...ACTIVATION }, actor }),
  ...
  for (const moduleName of DOMAIN_MODULES) {
    ...
    const retried = await app.runAction({ module: 'order', action: 'activate-contract', recordId: order.id, input: { ...POLICY, ...ACTIVATION }, actor });
    assert.equal(retried.result.counts.subscriptionLines, 3, `${moduleName}: retry completes`);
    const contract = app.modules.get('commercial-contract').service.listWhere({ orderId: order.id })[0];
    assert.equal(app.modules.get('contract-line').service.listWhere({ contractId: contract.id }).length, 5);
    assert.equal(app.modules.get('subscription-line').service.listWhere({ contractId: contract.id }).length, 3);
  }

  // ---- concurrency in one app ----
  const { order: raceOrder } = await signedOrder(root, app, { name: 'Race Deal' });
  const results = await Promise.allSettled([
    app.runAction({ module: 'order', action: 'activate-contract', recordId: raceOrder.id, input: { ...POLICY, ...ACTIVATION }, actor }),
    app.runAction({ module: 'order', action: 'activate-contract', recordId: raceOrder.id, input: { ...POLICY, ...ACTIVATION }, actor }),
  ]);
  ...
  const across = await Promise.allSettled([
    app.runAction({ module: 'order', action: 'activate-contract', recordId: twoConnection.id, input: { ...POLICY, ...ACTIVATION }, actor }),
    sibling.app.runAction({ module: 'order', action: 'activate-contract', recordId: twoConnection.id, input: { ...POLICY, ...ACTIVATION }, actor }),
  ]);
...
test('the contract reads independently of the catalog, the quote and live CRM rows', async (t) => {
  ...
  await client.module('order').action(order.id, 'activate-contract', { ...POLICY, ...ACTIVATION });
  const contract = app.modules.get('commercial-contract').service.listWhere({ orderId: order.id })[0];
  const version = app.modules.get('contract-version').service.listWhere({ contractId: contract.id })[0];
  const before = app.modules.get('contract-line').service.listWhere({ contractVersionId: version.id })
    .map((line) => `${line.componentKey}|${line.netAmountCents}|${line.tiersJson}|${line.commercialActivation}|${line.obligationsJson}`).sort();
  ...
  assert.equal(rendered.subscriptionLines.length, 3);
...
test('audit, events and trace are exact, and exact reads survive the list bound', async (t) => {
  ...
  assert.equal(auditFor('subscription-line') - before['subscription-line'], 3);
  ...
  await assert.rejects(
    () => client.module('order').action(failing.id, 'activate-contract', { ...POLICY, ...ACTIVATION, effectiveDate: '2026-09-02', termStartDate: '2026-09-01', termEndDate: '2027-08-31' }),
  ...
  for (let index = 0; index < 520; index += 1) {
    await contracts.createManaged({
      ...
      autoRenew: false, renewalNoticeDays: 0,
      termsSource: 'post-signature-operational-activation', termsReason: 'bulk fixture',
      policy: 'p', policyVersion: 1, policyFingerprint: 'f'.repeat(64),
...
test('hostile input stays inert data and never reaches a route or a prototype', async (t) => {
  ...
  await client.module('order').action(order.id, 'activate-contract', {
    ...POLICY, ...ACTIVATION,
    classificationOverrides: [
      { orderComponentId: ambiguous.orderComponentId, dimension: 'commercial', value: 'subscription', reason: nasty },
      { orderComponentId: ambiguous.orderComponentId, dimension: 'obligations', value: [], reason: 'nothing further is owed' },
    ],
  });
  const line = app.modules.get('contract-line').service.listWhere({ orderComponentId: ambiguous.orderComponentId })[0];
  assert.equal(line.commercialOverrideReason, nasty.trim(), 'stored verbatim as text, never interpreted');
  ...
  for (const suffix of [String.fromCharCode(0), String.fromCharCode(27), String.fromCharCode(127)]) {
    await assert.rejects(
      () => client.module('order').action(nul.id, 'activate-contract', {
        ...POLICY, ...ACTIVATION,
        classificationOverrides: [
          { orderComponentId: nulTarget.orderComponentId, dimension: 'commercial', value: 'subscription', reason: `hidden${suffix}tail` },
        ],
      }),
  ...
  await assert.rejects(
    () => client.module('order').action(other.id, 'activate-contract', {
      ...POLICY, ...ACTIVATION,
      classificationOverrides: JSON.parse('[{"orderComponentId":"x","dimension":"commercial","value":"subscription","reason":"r","__proto__":{"polluted":true}}]'),
    }),
  ...
  const response = await fetch(`${baseUrl}/api/modules/order/records/${other.id}/actions/activate-contract`, {
    ...
    body: JSON.stringify({ ...POLICY, ...ACTIVATION, effectiveDate: '2026-02-30' }),
  });
